...
const getResendConfig = () => {
	...
	return {
		...
		auditBookingLink: process.env.AUDIT_BOOKING_LINK?.trim() || '',
	};
};
...
export async function sendLeadEmails(body) {
	const { apiKey, from, auditBookingLink } = getResendConfig();
	...
	const qualificationDetailsHtml =
		!isWebsiteAudit &&
		(company_name ||
			website_url ||
			monthly_marketing_investment ||
			...
			? `
				...
				<p><strong>Website URL:</strong> ${website_url || 'N/A'}</p>
				<p><strong>Monthly Marketing Investment:</strong> ${
					monthly_marketing_investment || 'N/A'
				}</p>
				<p><strong>Project Timeline:</strong> ${project_timeline || 'N/A'}</p>
				<p><strong>What Prompted Them To Reach Out:</strong> ${
					action_prompt || 'N/A'
				}</p>
				<p><strong>Business Type:</strong> ${business_description || 'N/A'}</p>
				<p><strong>Growth Investment Acknowledgement:</strong> ${
					growth_investment_acknowledgement ? 'Yes' : 'No'
				}</p>
			`
			: '';
